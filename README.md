...
**Residual hole — measured.** Post-capture tampering with `a` (the post-attention output) is detected by the bridge / Merkle checks. A *consistent* fake-`a` forgery — where the adversary picks a fake `a_L` and then re-derives the rest of the trace honestly — clears the audit-only verifier. In the toy harness this is 10/10 verifier passes across {single-element, full-shift, zero-out} attacks at both layers, with 4/10 producing answer-changing forgeries. The honest reading is *"audited but not verified; post-hoc tampering is detected; consistent fake-`a` remains a known residual hole."* Closing the hole requires per-step Q retention (deferred) or a deterministic attention kernel (deferred). Methodology and results: [`docs/security/redteam_audit_only.md`](docs/security/redteam_audit_only.md).
